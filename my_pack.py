...
def builtin__file__(module_name: str="") -> pd.DataFrame|str:
    ...
    def module(name: str) -> str:
        path=df[df["module_name"]==name].iloc[0].tolist()[1:]
...
def copy_gen(gen: Generator) -> Generator:
    ...
    Note: alternatively you can use itertools.tee,
    however, using so will mean that you must use
    the generators provided from tee and not the
    original generator. Because of this even though
...
class pickle_stack:
    ...
    def _readonly_buffer(self,arg) -> Any:
        ...
        # changes made: returned as a value instead of appending
        """
        buf = self.stack.pop()
        with memoryview(buf) as m:
            if not m.readonly: return m.toreadonly()
